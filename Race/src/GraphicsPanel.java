...
//				g2.drawRect(c.getBounds().x, c.getBounds().y, (int)c.getBounds().getWidth(), (int)c.getBounds().getHeight());
	...
	public void clock() {
		...
		if(count1 == 1000 ) {  //every 10s 
			
			if((int)(Math.random()*2 + 1) == 1)
				cops.add(new Character(5, 1000, 100)); //instantiates cops 
			else
				cops.add(new Character(5, 1000, 180));
			count1 = 0;
		}
	...
	//switch that checks for keys pressed and performs actions accordingly
		...
		case KeyEvent.VK_SPACE:
			if((int)numbullet>0) {
				bullets.add(new Character(6, (int) jeep.getBounds().getX(), (int) jeep.getBounds().getY() - 10 ));
				numbullet--;
			}
			break;
		}
